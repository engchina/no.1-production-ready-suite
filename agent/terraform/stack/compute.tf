resource "oci_core_instance" "generated_oci_core_instance" {
  ...
  lifecycle {
    ...
    precondition {
      condition     = trimspace(var.app_basic_auth_password) != ""
      error_message = "app_basic_auth_password must be configured. Nginx protects the Agent Control Plane UI and API with HTTP Basic authentication."
    }
  }
}
