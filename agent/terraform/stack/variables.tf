...
variable "compartment_ocid" {
  description = "OCI compartment OCID for the Agent Control Plane deployment."
  type        = string
  default     = ""
}
...
variable "adb_workload" {
  description = "Autonomous AI Database workload type. Agent stores Runtime checkpoints and Run leases, so OLTP is the default."
  type        = string
  default     = "OLTP"
...
variable "application_git_url" {
  description = "Git repository URL for the Production Ready suite monorepo (contains agent/ and platform/)."
  type        = string
...
variable "application_git_ref" {
  description = "Git branch or tag of the Production Ready suite used to deploy the Agent Control Plane."
  type        = string
  default     = "main"
...
variable "app_basic_auth_user" {
  description = "HTTP Basic authentication user name that Nginx requires in front of the Agent Control Plane UI and API."
  type        = string
  default     = "agent_admin"

  validation {
    condition     = can(regex("^[A-Za-z][A-Za-z0-9_.-]{2,31}$", var.app_basic_auth_user))
    error_message = "app_basic_auth_user must start with a letter and contain 3-32 letters, digits, underscores, dots, or hyphens."
  }
}

variable "app_basic_auth_password" {
  description = "HTTP Basic authentication password that Nginx requires in front of the Agent Control Plane UI and API."
  type        = string
  sensitive   = true
  default     = ""
  ...
  validation {
    condition = (
      var.app_basic_auth_password == ""
      || (
        length(var.app_basic_auth_password) >= 12
        && length(var.app_basic_auth_password) <= 64
        && !can(regex("[\r\n]", var.app_basic_auth_password))
        && !can(regex("[\"':]", var.app_basic_auth_password))
        && can(regex("[0-9]", var.app_basic_auth_password))
        && can(regex("[a-z]", var.app_basic_auth_password))
        && can(regex("[A-Z]", var.app_basic_auth_password))
      )
    )
    error_message = "app_basic_auth_password must be 12-64 characters, include uppercase, lowercase, and digits, and not contain quotes, colons, or line breaks."
  }
}
