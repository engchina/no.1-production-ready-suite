...
from app.schemas.settings import (
    ...
    OciSettingsUpdate,
    RdfNetworkApplyData,
    RdfNetworkApplyRequest,
    RdfNetworkPlanData,
    RdfNetworkPlanStep,
    RdfNetworkSettingsData,
    RdfNetworkSettingsUpdate,
...
ORACLE_RDF_IDENTIFIER_RE = re.compile(r"^[A-Z][A-Z0-9_]{0,127}$")
ORACLE_RDF_OPTIONS_RE = re.compile(r"^[A-Z0-9_=\s]+$")
RDF_NETWORK_CONFIRMATION = "ADMIN_EXECUTE"
RDF_NETWORK_PLAN_VERSION = "V001"
...
@router.get(
    "/database/rdf-network",
    response_model=ApiResponse[RdfNetworkSettingsData],
)
def get_rdf_network_settings() -> ApiResponse[RdfNetworkSettingsData]:
    """Ontology publish 用 RDF network 設定と DDL なし status を取得する。"""

    return ApiResponse(data=_rdf_network_settings_data(get_settings()))
...
def update_rdf_network_settings(
    payload: RdfNetworkSettingsUpdate,
) -> ApiResponse[RdfNetworkSettingsData]:
    settings = get_settings()
    candidate = _rdf_network_settings_candidate(settings, payload)
    _persist_rdf_network_settings(candidate)
    _apply_rdf_network_settings(settings, candidate)
    close_oracle_pool()
    return ApiResponse(data=_rdf_network_settings_data(settings))


@router.get(
    "/database/rdf-network/plan",
    response_model=ApiResponse[RdfNetworkPlanData],
)
def get_rdf_network_plan() -> ApiResponse[RdfNetworkPlanData]:
    return ApiResponse(data=_rdf_network_plan_data(get_settings()))


@router.post(
    "/database/rdf-network/apply",
    response_model=ApiResponse[RdfNetworkApplyData],
)
def apply_rdf_network_plan(
    payload: RdfNetworkApplyRequest,
) -> ApiResponse[RdfNetworkApplyData]:
    settings = get_settings()
    plan = _rdf_network_plan_data(settings)
    if payload.confirmation != RDF_NETWORK_CONFIRMATION:
        raise HTTPException(
            status_code=409,
            detail=f"RDF network 作成には {RDF_NETWORK_CONFIRMATION} を入力してください。",
        )
    if payload.checksum != plan.checksum:
        raise HTTPException(
            status_code=409,
            detail="RDF network SQL plan のチェックサムが一致しません。状態を再取得してください。",
        )
    if not plan.configured:
        raise HTTPException(status_code=409, detail="RDF network 設定を保存してください。")
    if plan.manual_action_required:
        raise HTTPException(
            status_code=409,
            detail=(
                "RDF network owner が現在の ORACLE_USER と異なるため自動適用できません。"
                "表示された SQL を DBA または network owner で実行してください。"
            ),
        )
    if not plan.can_apply:
        raise HTTPException(status_code=409, detail="RDF network plan を適用できません。")

    current = _rdf_network_settings_data(settings)
    if current.status == "ready":
        return ApiResponse(
            data=RdfNetworkApplyData(status="already_configured", network=current)
        )
    _execute_rdf_network_create(settings)
    close_oracle_pool()
    return ApiResponse(
        data=RdfNetworkApplyData(
            status="applied",
            network=_rdf_network_settings_data(settings),
        )
    )
...
def _normalize_rdf_identifier(value: str, field_name: str, *, required: bool = False) -> str:
    normalized = value.strip().upper()
    if not normalized:
        if required:
            raise HTTPException(status_code=422, detail=f"{field_name} を入力してください。")
        return ""
    if not ORACLE_RDF_IDENTIFIER_RE.fullmatch(normalized):
        raise HTTPException(
            status_code=422,
            detail=f"{field_name} は英字で始まり、英数字とアンダースコアだけで入力してください。",
        )
    return normalized


def _normalize_rdf_options(value: str) -> str:
    normalized = " ".join(value.strip().upper().split())
    if not normalized:
        return ""
    if not ORACLE_RDF_OPTIONS_RE.fullmatch(normalized):
        raise HTTPException(
            status_code=422,
            detail=(
                "RDF network options は英数字、アンダースコア、等号、空白だけで"
                "入力してください。"
            ),
        )
    if any(token in normalized for token in (";", "--", "/*", "*/")):
        raise HTTPException(
            status_code=422,
            detail="RDF network options に SQL 記号は使用できません。",
        )
    return normalized


def _rdf_network_settings_candidate(
    base: Settings,
    payload: RdfNetworkSettingsUpdate,
) -> Settings:
    owner = _normalize_rdf_identifier(payload.network_owner, "network owner")
    network_name = _normalize_rdf_identifier(payload.network_name, "network name")
    tablespace = _normalize_rdf_identifier(payload.tablespace, "tablespace")
    options = _normalize_rdf_options(payload.options)
    if bool(owner) != bool(network_name):
        raise HTTPException(
            status_code=422,
            detail="RDF network owner と network name は両方入力するか、両方空にしてください。",
        )
    if tablespace == "SYSTEM":
        raise HTTPException(
            status_code=422,
            detail="RDF network tablespace に SYSTEM は指定できません。",
        )
    return base.model_copy(
        update={
            "nl2sql_ontology_rdf_network_owner": owner,
            "nl2sql_ontology_rdf_network_name": network_name,
            "nl2sql_ontology_rdf_tablespace": tablespace,
            "nl2sql_ontology_rdf_network_options": options,
        }
    )


def _apply_rdf_network_settings(target: Settings, source: Settings) -> None:
    target.nl2sql_ontology_rdf_network_owner = source.nl2sql_ontology_rdf_network_owner
    target.nl2sql_ontology_rdf_network_name = source.nl2sql_ontology_rdf_network_name
    target.nl2sql_ontology_rdf_tablespace = source.nl2sql_ontology_rdf_tablespace
    target.nl2sql_ontology_rdf_network_options = source.nl2sql_ontology_rdf_network_options


def _persist_rdf_network_settings(settings: Settings) -> None:
    _write_env_values(
        BACKEND_ENV_FILE,
        {
            "NL2SQL_ONTOLOGY_RDF_NETWORK_OWNER": settings.nl2sql_ontology_rdf_network_owner,
            "NL2SQL_ONTOLOGY_RDF_NETWORK_NAME": settings.nl2sql_ontology_rdf_network_name,
            "NL2SQL_ONTOLOGY_RDF_TABLESPACE": settings.nl2sql_ontology_rdf_tablespace,
            "NL2SQL_ONTOLOGY_RDF_NETWORK_OPTIONS": (
                settings.nl2sql_ontology_rdf_network_options
            ),
        },
        section_comment="# NL2SQL Ontology RDF Network",
        error_detail="RDF network 設定を backend/.env へ保存できませんでした。",
    )


def _rdf_network_config_values(settings: Settings) -> tuple[str, str, str, str]:
    owner = _normalize_rdf_identifier(settings.nl2sql_ontology_rdf_network_owner, "network owner")
    network_name = _normalize_rdf_identifier(
        settings.nl2sql_ontology_rdf_network_name,
        "network name",
    )
    tablespace = _normalize_rdf_identifier(settings.nl2sql_ontology_rdf_tablespace, "tablespace")
    options = _normalize_rdf_options(settings.nl2sql_ontology_rdf_network_options)
    return owner, network_name, tablespace, options


def _current_oracle_user(settings: Settings) -> str:
    return settings.oracle_user.strip().upper()


def _rdf_network_settings_data(settings: Settings) -> RdfNetworkSettingsData:
    owner, network_name, tablespace, options = _rdf_network_config_values(settings)
    current_user = _current_oracle_user(settings)
    configured = bool(owner and network_name)
    warnings: list[str] = []
    tablespace_can_apply = bool(tablespace and tablespace != "SYSTEM")
    if tablespace == "SYSTEM":
        warnings.append("SYSTEM tablespace は使用できません。")
    if bool(owner) != bool(network_name):
        warnings.append("RDF network owner と network name は両方設定してください。")
        return RdfNetworkSettingsData(
            network_owner=owner,
            ...
            mode="local_fallback",
            status="unavailable",
            current_oracle_user=current_user,
            can_apply=False,
            message_ja="RDF network 設定が不完全です。",
    ...
    if not configured:
        return RdfNetworkSettingsData(
            ...
            can_apply=False,
            message_ja=(
                "RDF network は未設定です。Ontology publish は local OWL2RL と "
                "SHACL Core 検証で公開します。"
            ),
            warnings_ja=warnings,
        )
    manual_action_required = bool(current_user and owner != current_user)
    if manual_action_required:
        warnings.append(
            "network owner が現在の ORACLE_USER と異なるため、"
            "作成は DBA または owner で手動実行してください。"
        )
    try:
        metadata = _load_rdf_network_metadata(owner, network_name)
    except Exception as exc:
        codes = _oracle_error_codes(str(exc))
        ...
        if manual_action_required:
            return RdfNetworkSettingsData(
                ...
                message_ja=(
                    "RDF network の自動作成対象外です。SQL plan を DBA または "
                    "network owner で実行してください。"
                ),
                warnings_ja=warnings,
                metadata={"error_code": ", ".join(codes) or None},
            )
        return RdfNetworkSettingsData(
            network_owner=owner,
            network_name=network_name,
            ...
            status="missing",
            current_oracle_user=current_user,
            can_apply=tablespace_can_apply,
            message_ja=f"RDF network を確認できません。未作成または権限不足です{code_suffix}。",
            warnings_ja=warnings,
            metadata={"error_code": ", ".join(codes) or None},
        )
    return RdfNetworkSettingsData(
        ...
        message_ja=(
            "RDF network は利用可能です。Ontology publish は Oracle RDF staging を使います。"
        ),
        warnings_ja=warnings,
        metadata=metadata,
    )
...
def _load_rdf_network_metadata(owner: str, network_name: str) -> dict[str, str | int | bool | None]:
    parameter_table = f"{owner}.{network_name}#RDF_PARAMETER"
    with (
        get_oracle_pool_manager().control_connection() as connection,
        ...
        row = cursor.fetchone()
    return {
        "parameter_rows": int(row[0]) if row and row[0] is not None else 0,
        "parameter_table": parameter_table,
    }


def _rdf_network_create_statement() -> str:
    return (
        "BEGIN\n"
...
def _rdf_network_display_sql(
    *,
    ...
    return (
        ...
        f"    options => {('NULL' if not options else repr(options))},\n"
        f"    network_owner => '{owner}',\n"
...
def _rdf_network_plan_checksum(sql: str) -> str:
    return hashlib.sha256(
        canonical_json_like({"version": RDF_NETWORK_PLAN_VERSION, "sql": sql}).encode("utf-8")
    ).hexdigest()


def canonical_json_like(value: Mapping[str, str]) -> str:
    return json.dumps(value, ensure_ascii=False, sort_keys=True, separators=(",", ":"))


def _rdf_network_plan_data(settings: Settings) -> RdfNetworkPlanData:
    owner, network_name, tablespace, options = _rdf_network_config_values(settings)
    configured = bool(owner and network_name)
    current_user = _current_oracle_user(settings)
    manual_required = bool(configured and current_user and owner != current_user)
    warnings: list[str] = []
    if not configured:
        warnings.append("RDF network owner と network name を保存すると SQL plan を生成できます。")
    if configured and not tablespace:
        warnings.append("RDF network を作成する tablespace を入力してください。")
    if tablespace == "SYSTEM":
        warnings.append("SYSTEM tablespace は使用できません。")
    if manual_required:
        warnings.append("owner が現在の ORACLE_USER と異なるため自動適用できません。")
    sql = (
        _rdf_network_display_sql(
            owner=owner,
            network_name=network_name,
            tablespace=tablespace,
            options=options,
        )
        if configured and tablespace
        else ""
    )
    checksum = _rdf_network_plan_checksum(sql)
    can_apply = bool(configured and tablespace and not manual_required and tablespace != "SYSTEM")
    step_status: Literal["pending", "manual_required", "blocked", "ready"]
    if manual_required:
        step_status = "manual_required"
    elif can_apply:
        step_status = "pending"
    elif configured:
        step_status = "blocked"
    ...
    return RdfNetworkPlanData(
        configured=configured,
        can_apply=can_apply,
        manual_action_required=manual_required,
        confirmation_phrase=RDF_NETWORK_CONFIRMATION,
        checksum=checksum,
        steps=steps,
        warnings_ja=warnings,
    )
...
def _execute_rdf_network_create(settings: Settings) -> None:
    owner, network_name, tablespace, options = _rdf_network_config_values(settings)
    current_user = _current_oracle_user(settings)
    if not owner or not network_name or not tablespace:
        raise HTTPException(status_code=409, detail="RDF network 設定が不足しています。")
    if current_user and owner != current_user:
        raise HTTPException(
            status_code=409,
            detail="現在の ORACLE_USER 以外の owner には作成できません。",
        )
    if tablespace == "SYSTEM":
        raise HTTPException(status_code=422, detail="SYSTEM tablespace は使用できません。")
    try:
        with get_oracle_pool_manager().control_connection() as connection:
            with connection.cursor() as cursor:
                cursor.execute(
                    _rdf_network_create_statement(),
                    {
        ...
        raise HTTPException(
            status_code=409,
            detail=f"RDF network の作成に失敗しました{code_suffix}。",
        ) from exc
