...
- **工程を進める操作**: 対象情報の取得、SQL 生成、AI 抽出など、入力を確定して次工程へ進む操作は局所ツールではなく主アクションとして `size="lg"` にする。`ContentActionBar` で説明と操作を並べる場合も同様。同じバーの再試行ボタンも `lg` に揃える。コメント/アノテーション/ドメイン管理、オントロジー情報取得、ビューの JOIN/WHERE 抽出、生成データの対象テーブル取得へ共通適用する。
