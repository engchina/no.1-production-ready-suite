...
| `lg` | desktop 40px / 640px 未満 44px | 15–16 | **主アクションバー**(設定の保存/テスト/リセット、検索実行)・工程を進める取得/生成/抽出・ページ主 CTA |

- **同一アクションバー内のボタンは size を揃える**(例: 保存=lg なら隣のテスト/リセットも lg)。
- `lg` の実寸はアプリ共通 `Button` / `buttonVariants` が管理する。日本語テーマの root font-size は 14px のため、`h-10` は実際には 35px になる。ページ側で `h-10` / `sm:h-10` を上書きせず `size="lg"` を指定する。
