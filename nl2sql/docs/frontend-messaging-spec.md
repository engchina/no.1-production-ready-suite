...
- JSON／必須／数値入力の失敗は `FieldError` で該当入力に関連付ける。公開の阻害事項と無効理由を画面に残し、変更適用・公開・切戻し・能力実行では対象 Profile と実際の版を共通確認ダイアログで示す。
- 公開能力の前回結果は実行時の版と日時を表示し、現在の公開版を代入しない。応答喪失時は保存した操作の結果を照会し、自動再実行しない。
