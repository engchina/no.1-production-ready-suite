...
資料の業務記述不足や根拠の照合不可は、構築完了時の警告欄に表示せず、生成する Markdown の「記述範囲と補足」に重複なく保持する。記述可能な概念定義はそのまま生成し、不明な意味や仕様を創作しない。定義検証の warning も補足に分離し、error と定義競合は「解決が必要な定義の競合」に保持する。抽出失敗などの実行上の警告と公開版の検証記録は引き続き参照できる。

既存ジョブの同種警告は表示から除外する。保存済み Markdown や公開版は書き換えず、再構築した下書きから新しい注記構成を適用する。
