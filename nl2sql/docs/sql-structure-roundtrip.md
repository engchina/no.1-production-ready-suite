...
- `業務質問を生成` 完了後は `SQL論理構造` タブへ移り、表示中の作業ではタブへフォーカスを移す。
- AI の完全な `logical_structure` を編集欄へ表示する。古い決定論の `logical_structure_items` で覆い隠さない。
- 各生成段階の JSON を Pydantic で検証する。質問生成だけ失敗しても完成した論理構造は残し、簡易質問であることを警告する。
...
自動テストは Enterprise AI と API を fake/mock 化した契約検証である。実モデルと Oracle のデータで結果の同値性を比較する live 受入は別途必要。任意の SQL について「100% 同値」を証明したものではない。
