...
function CapabilityCard({ profileId, releaseId, capability, implementations, refresh, stale }: { profileId: string; releaseId: string; capability: Capability; implementations: string[]; refresh: () => Promise<unknown>; stale: boolean }) {
  ...
  const [pendingJson, setPendingJson] = useWorkspaceState(`ontology-capability:${profileId}:${definition.id}:pending`, "");
  const pending: PendingAction | null = pendingJson ? JSON.parse(pendingJson) as PendingAction : null;
  ...
  const run = async (action: "bind" | "invoke" | "preview" | "execute") => {
    ...
    const operation = action === "execute" ? pending ?? (preview ? { preview, key: crypto.randomUUID(), input, releaseId } : null) : null;
    ...
    if (action === "execute" && pending) {
      ...
      if (current !== consent.current || recovered.isError || recovered.data?.status !== "unresolved") return;
    }
    if (["bind", "execute", "invoke"].includes(action) && !await confirm({ title: t(`ontologyCapability.${action}`), description: t("ontologyWorkspace.confirmScope", { profile: profileId, version: operation?.releaseId ?? releaseId }), confirmLabel: t("ontologyWorkspace.confirm"), tone: "info" })) return;
    ...
    try {
      if (action === "bind") {
        ...
        const body = action === "execute" ? { preview_id: operation?.preview.id, confirmed: true } : { release_id: releaseId, parameters: convert(definition.parameters ?? [], values), target: convert(capability.target_parameters ?? [], targets) };
      ...
      setPreview(null); setError(cause instanceof Error ? cause.message : t("ontologyWorkspace.failed")); }
    finally { setBusy(""); }
  };
  ...
  return <article className="grid min-w-0 grid-cols-1 gap-3 rounded border border-border p-3" aria-label={`${definition.name_ja} (${definition.api_name})`}>
    ...
    {capability.status === "available" ? <><fieldset className="grid min-w-0 gap-3"><legend className="mb-2 text-sm font-medium">{t("ontologyCapability.parameters")}</legend>{fields(definition.parameters ?? [], values, setValues)}{fields(capability.target_parameters ?? [], targets, setTargets)}</fieldset><Button size="md" variant="secondary" disabled={!!busy || stale || !!pending} loading={busy === "preview" || busy === "invoke"} onClick={() => void run(isFunction ? "invoke" : "preview")}><span className="block w-full whitespace-normal break-words">{t(isFunction ? "ontologyCapability.invoke" : "ontologyCapability.preview")}</span></Button></> : null}
    {pending ? <div className="grid min-w-0 gap-3" role="status">
      <p className="text-sm">{t("ontologyCapability.pending")}</p>
      <p className="break-all text-xs">{t("ontologyWorkspace.published")}: {pending.releaseId}</p>
      <div className="overflow-auto"><table className="w-full text-left text-sm"><thead><tr><th>{t("ontologyCapability.property")}</th><th>{t("ontologyCapability.before")}</th><th>{t("ontologyCapability.after")}</th></tr></thead><tbody>{Object.keys(pending.preview.before).map(name => <tr key={name}><td className="p-2">{name}</td><td className="p-2">{String(pending.preview.before[name] ?? "")}</td><td className="p-2">{String(pending.preview.after[name] ?? "")}</td></tr>)}</tbody></table></div>
      {outcome.isError ? <FormStatus tone="danger" message={t("ontologyWorkspace.loadError")} /> : null}
      <Button size="md" variant="secondary" disabled={!!busy} loading={outcome.isFetching} onClick={() => void outcome.refetch()}><span className="block w-full whitespace-normal break-words">{t("ontologyCapability.checkOutcome")}</span></Button>
      <Button size="lg" variant="primary" disabled={!!busy || outcome.isFetching || outcome.isError || outcome.data?.status !== "unresolved"} onClick={() => void run("execute")}><span className="block w-full whitespace-normal break-words">{t("ontologyCapability.retryOriginal")}</span></Button>
    </div> : null}
    {preview && !pending ? <div className="grid min-w-0 gap-3"><p className="text-xs">{t("ontologyCapability.expires")}: {preview.expires_at}</p><div className="overflow-auto"><table className="w-full text-left text-sm"><thead><tr><th>{t("ontologyCapability.property")}</th><th>{t("ontologyCapability.before")}</th><th>{t("ontologyCapability.after")}</th></tr></thead><tbody>{Object.keys(preview.before).map(name => <tr key={name}><td className="p-2">{name}</td><td className="p-2">{String(preview.before[name] ?? "")}</td><td className="p-2">{String(preview.after[name] ?? "")}</td></tr>)}</tbody></table></div><Button size="lg" variant="primary" disabled={!!busy || stale} loading={busy === "execute"} onClick={() => void run("execute")}><span className="block w-full whitespace-normal break-words">{t("ontologyCapability.execute")}</span></Button></div> : null}
