...
export function orderedBuildProgress(job:OntologyBuildJob, steps:Progress[]):Progress[] {
  ...
  return groups.filter(([, ,children])=>children.length).map(([id,label,children])=>{
    ...
    return {id,testId:`ontology-build-stage-${id}`,dataStatus:status === "done" ? "succeeded" : status === "error" ? "failed" : status,label:t(label),status:status as WorkflowProgressStepStatus,statusLabel:t(`profiles.ontologyBuild.stepStatus.${status === "done" ? "succeeded" : status === "error" ? "failed" : status}`),open:status === "running" || status === "error",content:<div className="grid gap-3">{id === "supplement" && <div className="grid gap-2 text-sm">{[conceptKinds.slice(0,6),conceptKinds.slice(6)].map((kinds,i)=><details key={i}><summary>{t(i?"markdownOntology.auxConcepts":"markdownOntology.mainConcepts")}</summary><ul className="grid gap-1 pl-4">{kinds.map(k=><li key={k}>{conceptLabel(k)} · {job.concept_coverage?.find(c=>c.kind===k)?.count ? t("ontologyResults.phaseStatus.succeeded") : job.concept_coverage?.find(c=>c.kind===k)?.reason_ja || t("ontologyResults.phaseStatus.pending")}</li>)}</ul></details>)}</div>}{children.map(s=><details key={s.id} open={s.open} data-testid={s.testId} data-step-status={s.dataStatus} className="min-w-0"><summary className="cursor-pointer text-sm">{s.label} · {s.statusLabel}{s.elapsedLabel ? ` · ${s.elapsedLabel}` : ""}</summary><p className="text-sm text-fg-muted">{s.description}</p>{s.content}</details>)}</div>};
  });
}
