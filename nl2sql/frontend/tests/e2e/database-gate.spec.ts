...
async function expectUnifiedDatabaseGate(page: Page) {
  await expect(
    page.getByRole("heading", { name: "データベースを起動してください" })
  ).toBeVisible();
  await expect(page.getByText(DATABASE_UNAVAILABLE_MESSAGE, { exact: true })).toBeVisible();
...
test("DB 到達不可では Profile を空表示せず ADB 管理へ誘導する", async ({ page }) => {
  ...
  const settingsLink = await expectUnifiedDatabaseGate(page);
...
test("STOPPED から起動し AVAILABLE 後に元の Profile へ戻って復元データを表示する", async ({
  ...
  await expect(page.getByText("状態: 起動済み")).toBeVisible({ timeout: 15_000 });
...
test("バックエンドの状態確認失敗でも設定入口と再試行を残す", async ({ page }) => {
  ...
  await expectUnifiedDatabaseGate(page);
});
