...
test("AI オントロジー構築の実行 → 進捗 → 提案承認 → 公開の導線が機能する", async ({ page }, testInfo) => {
  ...
  if (testInfo.project.name === "desktop") {
    ...
    expect(Math.abs((businessTextareaBox?.y ?? 0) - (sourcePanelBox?.y ?? 0))).toBeLessThan(48);
  }
  ...
  // URL 復元可能な「業務モデル」tab へ切り替える
  await page.getByRole("tab", { name: "業務モデル" }).click();
...
test("利用 tab で推薦確認と同一 revision のコンテキストを確認できる", async ({
  page,
}) => {
  await mockApi(page);
    ...
    candidates: [
      {
        ...
        ontology_revision_id: "revision-1",
        score: 0.92,
        matched_scenarios_ja: ["受注状況を分析する"],
        matched_terms: ["受注"],
  ...
  await page.route("**/api/nl2sql/ontology/profile-recommendations", (route) =>
    fulfillJson(route, { recommendation })
  );
  await page.route("**/api/nl2sql/ontology/profile-recommendations/*/confirm", (route) =>
    fulfillJson(route, {
      recommendation: {
        ...recommendation,
  ...
  await page.route("**/api/nl2sql/profiles/*/ontology-context/search", (route) =>
    fulfillJson(route, {
      profile_id: "default",
      profile_view_id: "profile-view:default",
      ontology_revision_id: "revision-1",
      hits: [],
      nodes: ontologyView.ontology_graph.nodes,
      edges: ontologyView.ontology_graph.edges,
      mermaid: "graph TD\n  ORDERS --> CUSTOMERS",
      llm_markdown: "# NL2SQL Ontology Context\n\n- Revision: `revision-1`",
      owl_turtle: "@prefix owl: <http://www.w3.org/2002/07/owl#> .",
  ...
  await page.goto("/ontology-build?profile=default&tab=usage");
  await expect(page.getByRole("tab", { name: "利用・コンテキスト" })).toHaveAttribute(
    "aria-selected",
    "true"
  );
  await page.getByLabel("テストする質問").fill("受注状況を確認したい");
  await page.getByRole("button", { name: "推薦候補を表示" }).click();
  await expect(
    page.getByRole("list", { name: "Profile 推薦候補" }).getByText("既定プロファイル")
  ).toBeVisible();
  await page.getByRole("button", { name: "この Profile を確認" }).click();
  await expect(page.getByText("Profile の選択を確認しました。")).toBeVisible();

  await page.getByRole("button", { name: "必要なコンテキストを生成" }).click();
  await expect(page.getByText("Revision: `revision-1`", { exact: false })).toBeVisible();
  await page.getByRole("tab", { name: "OWL Turtle" }).click();
  await expect(page.getByText("@prefix owl", { exact: false })).toBeVisible();
  const overflow = await page.evaluate(
    () => document.documentElement.scrollWidth > window.innerWidth
  );
  expect(overflow).toBe(false);
