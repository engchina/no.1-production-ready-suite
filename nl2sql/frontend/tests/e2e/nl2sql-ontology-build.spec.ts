...
test("AI オントロジー構築の実行 → 進捗 → 提案承認 → 公開の導線が機能する", async ({ page }, testInfo) => {
  ...
  await expect(section.getByText("AI 構築が完了しました", { exact: false })).toBeVisible({
    timeout: 15000,
  });
  expect(state.startPayloadSeen).toBe(true);
  await expect(steps.getByText("完了").first()).toBeVisible();
  ...
  await expect(section.getByText("Ontology を公開しました。")).toBeVisible();
...
test("送信直後にプレースホルダーが出て、すべて承認で一括承認できる", async ({ page }) => {
  ...
  const section = page.getByTestId("profile-ontology-build");
  await section.getByRole("button", { name: "AI 構築を実行" }).click();
  await expect(page.getByTestId("ontology-build-submitting")).toBeVisible();
