...
test("検索を実行すると実処理の段階別進捗と結果を表示する", async ({ page }) => {
  ...
  await expect(generate.getByRole("button", { name: "コピー" })).toBeVisible();
  await expect(page.getByRole("cell", { name: "青山商事" })).toBeVisible();
...
test("direct SQL page executes SELECT SQL from the sidebar menu", async ({ page }) => {
  const api = await mockNl2SqlApi(page);

  // サイドナビ「SQL 直接実行」から専用ページへ遷移する。
  await page.goto("/query");
  await page.getByRole("link", { name: /SELECT SQL を直接実行/ }).first().click();
...
test("AI 活用の 4 画面はナビ切替で入力を保持し、リセットで消える", async ({ page }) => {
  ...
  // SPA ナビで SQL 直接実行へ移動し、そちらにも入力する。
  await page.getByRole("link", { name: /SELECT SQL を直接実行/ }).first().click();
  await expect(page).toHaveURL(/\/direct-sql$/);
...
test("root route opens SQL generation and sidebar exposes feature surfaces", async ({ page }) => {
  ...
  await expect(page.getByRole("link", { name: /SELECT SQL を直接実行/ }).first()).toBeVisible();
