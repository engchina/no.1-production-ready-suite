...
test("サイドバーを producer / consumer 思想のユーザー向け 4 セクションで表示する", async ({ page }) => {
  ...
  for (const label of ["フィードバック学習", "質問学習", "品質評価", "エンジン運用", "モデル学習", "データ運用"]) {
    await expect(sidebar.getByText(label, { exact: true })).toBeVisible();
  }
...
test("セクション折りたたみで所属項目だけを隠し、他セクションは維持する", async ({ page }) => {
  ...
  await expect(sidebar.getByText("質問学習", { exact: true })).toBeVisible();
...
test("セクション見出しはキーボードで開閉できる", async ({ page }) => {
  ...
  await expect(sidebar.getByText("質問学習", { exact: true })).toBeHidden();
