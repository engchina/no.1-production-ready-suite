...
async function mockApi(page: Page) {
  await page.route("**/api/**", async (route) => {
    await route.fulfill({
      ...
      body: JSON.stringify({ data: null, error_messages: [], warning_messages: [] }),
    });
  });
}
...
test("サイドバーを producer / consumer 思想のユーザー向け 4 セクションで表示する", async ({ page }) => {
  ...
  for (const label of ["ダッシュボード", "スキーマ管理", "データ投入", "業務プロファイル", "用語・ルール"]) {
    await expect(sidebar.getByText(label, { exact: true })).toBeVisible();
  }

  for (const label of ["SQL 生成", "SQL 確認・修復", "実行履歴"]) {
    await expect(sidebar.getByText(label, { exact: true })).toBeVisible();
  }

  for (const label of ["フィードバック学習", "品質評価", "エンジン運用", "接続診断", "モデル学習", "データ運用"]) {
    await expect(sidebar.getByText(label, { exact: true })).toBeVisible();
  }
...
test("セクション折りたたみで所属項目だけを隠し、他セクションは維持する", async ({ page }) => {
  ...
  await expect(sidebar.getByText("スキーマ管理", { exact: true })).toBeHidden();
