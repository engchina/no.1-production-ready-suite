...
test("role editor saves profile access and supports bulk selection", () => {
  ...
  assert.match(rolesPageSource, /allowed_profile_ids: draftGrantsAllProfileAccess \? \[\] : draft\.allowedProfileIds/u);
...
test("role editor handles system admin and empty profile states", () => {
  assert.match(rolesPageSource, /const PROFILE_MANAGE_PERMISSION = "nl2sql\.profiles\.manage"/u);
  assert.match(rolesPageSource, /roleGrantsAllProfileAccess/u);
  assert.match(rolesPageSource, /draftGrantsAllProfileAccess/u);
  assert.match(rolesPageSource, /profileAccessReadOnly/u);
  assert.match(rolesPageSource, /security\.roles\.profileAccessSystemAdmin/u);
  assert.match(rolesPageSource, /security\.roles\.profileAccessManagedAll/u);
