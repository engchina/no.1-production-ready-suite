...
文書は中身に加えて **処理レシピ(preprocess / parser / chunking)を自身のプロパティ**として持つ。1 文書 = 1 配信レシピ(= serving chunk_set 1 つ)。
...
レシピの既定値は **global(「検索・回答設定」配下の ファイル準備 / 文書解析 / 文書分割 など)** から解決し、文書が選んだ値で上書きする。**KB からは解決しない**。embedding / HNSW は単一固定。
...
同じ文書の検索精度を上げたい場合は **「別レシピで候補 chunk_set を materialize → 配信中(serving)と横並び比較 → 勝者を serving に昇格(敗者は GC)」** の実験で行う。本番に複数レシピを常駐させない(routed / fused は不採用)。定常状態は **1 文書 = 1 chunk_set**。KB membership を変えてもレシピ集合は変わらない。
