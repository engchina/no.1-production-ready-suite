...
外部 parser(docling / marker / unstructured / mineru / dots_ocr)は **backend と同居させず、
それぞれ独立した FastAPI マイクロサービス**(`services/parsers/<name>`)で動かす。各サービスは
...
- **mineru / dots_ocr は GPU(CUDA)で実 OCR を行う実 parser**(従来の「実 OCR は Enterprise AI
  VLM へ再マップ」方針からの逸脱。ユーザ明示要望による)。GPU 必須のため compose は
