...
def test_guardrail_static_readiness_does_not_call_oci(
    monkeypatch: MonkeyPatch, tmp_path: Path
) -> None:
    _write_oci_files(tmp_path, monkeypatch)
    settings = get_settings()
    ...
    assert settings_router._oci_guardrails_configured(settings) is True
    result = settings_router._test_oci_config(settings, verify_with_oci=False)
