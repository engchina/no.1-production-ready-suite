...
理由のある例外だけを残す（#147 で業務ビュー・フィードバック・ランタイム知識・回答プロンプト・保存された回答は規約に揃えた）。
...
| サービス管理 | ログ・ビルド・起動・停止・削除 | サービスごとの運用のコントロールパネルで、起動 / 停止を常に見せる必要がある。起動・停止を常に表示する主操作として残すか、`RowActionMenu` + `ObjectActionBar` にまとめるかは利用者の判断が必要なため、#147 では変えない（判断が出たら揃える）。 |
