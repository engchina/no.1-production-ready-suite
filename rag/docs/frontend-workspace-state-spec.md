...
| 業務ビュー（作成・編集のエディタ） | 名前・説明・設定の差分（KB は集合として比べる）。下書きはこのタブに残るため、確認は「移動する」。パンくず・サイドナビに加え、エディタの `一覧へ戻る` も同じ確認を通す |
...
ブラウザの戻る / 進む（`popstate`）も、data router の `useBlocker` で確認する（#138）。キャンセルすると URL と入力が残る。
...
復元した対象（業務ビュー・KB の絞り込み）が一覧に無くなった場合は、その選択だけを外し、別の対象へ置き換えない。URL の `?id=` の業務ビューが無い場合は「対象が見つかりません」と一覧へ戻る導線を出し、別の業務ビューを開かない。チャットの会話が取得できない場合は、選択を残してエラーと再試行を示す。

## 検証

- Playwright：`e2e/workspace-state.spec.ts`（desktop / mobile-375）。離脱の確認とキャンセル・破棄、dirty でないときの自由な移動、`beforeunload`、保存後の基準更新、業務ビューの新規の下書きの一覧からの再開（対象ごとの下書きと `?id=` の戻る / 進むは `e2e/business-views.spec.ts`）、検索・チャット・文書インデックスのページ往復と再読込、フィードバックの URL の復元
