...
| `PATCH` | `/api/knowledge-bases/{knowledge_base_id}` | 名前、説明、既定検索モード、検索設定を更新。 |
...
- 詳細検索設定: 折りたたみ領域。`top_k`、`rerank_top_n` などを数値 input で設定。
...
- 既定ナレッジベース作成・既存文書 backfill script。
- staging で category migration option を検証。
- 監査 table migration。
- README / docs/rag-architecture.md / backend README を更新。
