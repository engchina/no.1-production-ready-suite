...
for (const viewport of [
  ...
  test(`Agentic 設定は計画プロファイルを表示する (${viewport.name})`, async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: "クエリ計画プロファイル" })).toBeVisible();
    ...
    await expect(page.getByRole("link", { name: "Agentic アダプター" })).toHaveAttribute(
      "aria-current",
      "page"
    );
...
test("Agentic 設定は off 以外で LLM 追加呼び出し警告を出して保存できる", async ({ page }) => {
  ...
  await expect(page.getByText("Agentic プロファイルを保存しました。")).toBeVisible();
  expect(saved).toEqual({ profile: "decompose" });
  await expectNoHorizontalOverflow(page);
});

test("Agentic 設定取得に失敗したら再試行できる", async ({ page }) => {
  await page.setViewportSize({ width: 1280, height: 760 });
  await page.route("**/api/settings/agentic", async (route) => {
    await route.fulfill({
      status: 503,
      json: {
        data: null,
        error_messages: ["Agentic アダプター設定を取得できませんでした。"],
        warning_messages: [],
      },
    });
  });
  ...
  await expect(page.getByRole("alert")).toContainText("Agentic アダプター設定を取得できませんでした。");
...
function agenticEnvelope(profile: string) {
  const specs = [
    { name: "off", enabled: false, rewrite: false, decompose: false, multi_hop: false },
    { name: "query_rewrite", enabled: true, rewrite: true, decompose: false, multi_hop: false },
    { name: "decompose", enabled: true, rewrite: false, decompose: true, multi_hop: false },
    { name: "multi_hop", enabled: true, rewrite: false, decompose: true, multi_hop: true },
  ];
