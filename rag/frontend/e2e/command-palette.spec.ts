...
test("Cmd/Ctrl+K で開き、絞り込み → Enter で対象ページへ遷移する", async ({ page }) => {
  ...
  await input.fill("検索方法");

  // 絞り込み結果に検索方法が含まれる。
  await expect(dialog.getByRole("option", { name: /検索方法/ })).toBeVisible();
...
test("サイドバーの検索トリガーから開ける（375px・タッチ導線）", async ({ page }) => {
  ...
  await dialog.getByRole("option", { name: "品質評価 業務ビュー" }).click();
...
test("クリアボタンで入力をリセットし、件数フッターを更新する", async ({ page }) => {
  ...
  // 全件 = NAV_SECTIONS の総項目数(業務ビュー4[チャット含む]+ナレッジ構築4+検索・回答設定13[概要+12工程]+システム設定6=27)。
  await expect(dialog.getByText("27 件", { exact: true })).toBeVisible();
});
