...
test("文書 workspace で chunk と構造化 block を相互に確認できる", async ({ page }) => {
  ...
  // 左ペインのプレビューは常時表示。右ペインは抽出本文 / Chunk / エクスポートのタブ切替。
  await expect(page.getByRole("heading", { name: "原本プレビュー" })).toBeVisible();
  await expect(page.getByRole("tab", { name: "抽出本文" })).toBeVisible();
  ...
  // 抽出本文タブ: 直前の chunk 選択で紐づく element(tbl-1)が選択済み。
  await page.getByRole("tab", { name: "抽出本文" }).click();
