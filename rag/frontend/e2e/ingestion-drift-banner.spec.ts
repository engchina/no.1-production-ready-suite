...
async function mockWorkspace(page: Page, drift: boolean, parserDrift = false) {
  const knowledgeBases = [{ id: "kb-1", name: "社内規程" }];
  await page.route("**/api/knowledge-bases**", (route) =>
    route.fulfill({
      json: ok({ items: knowledgeBases, total: 1, limit: 100, offset: 0, has_next: false }),
    })
  );
  // 3 層モデル: ingestion-config は文書単位の単一レシピ(effective=global)とドリフトのみ。
...
test("ドリフトが無ければバナーを表示せず、文書の単一レシピを表示する", async ({ page }) => {
  ...
  // 3 層モデル: KB 別グルーピングではなく「この文書のレシピ」を 1 枚で表示する。
  const recipe = page.getByRole("region", { name: "この文書のレシピ" });
  await expect(recipe).toBeVisible();
  await expect(recipe).toContainText("Office→PDF");
  await expect(recipe).toContainText("ページ単位");
  // 単一レシピカードには KB リンク(per-KB グルーピング)が無い。
  await expect(recipe.getByRole("link")).toHaveCount(0);
});
...
test("構築設定の読込中と取得失敗を文書画面内で通知する", async ({ page }) => {
  ...
  await expect(page.getByRole("status", { name: "構築設定を読み込んでいます…" })).toBeVisible();
  const recipe = page.getByRole("region", { name: "この文書のレシピ" });
  await expect(recipe.getByText("構築設定を取得できません", { exact: true })).toBeVisible();
  await expect(recipe.getByRole("button", { name: "再試行" })).toBeVisible();
});
