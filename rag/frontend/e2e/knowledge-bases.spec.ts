...
test("知識ベース管理で作成、文書追加、文書解除、アーカイブができる", async ({ page }) => {
  ...
  await expect(page.getByRole("button", { name: "社内規程" })).toBeVisible();
  ...
  await expect(page.getByText("知識ベースを作成しました。").first()).toBeVisible();
  await expect(page.getByRole("button", { name: "設計資料" })).toBeVisible();
  ...
  await expect(page.getByRole("button", { name: "設計資料" })).toHaveCount(0);
  await expectNoPageOverflow(page);
});
