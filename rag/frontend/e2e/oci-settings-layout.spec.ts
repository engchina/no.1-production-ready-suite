...
for (const viewport of [
  ...
  test(`OCI 設定カードのアクション文言が収まる (${viewport.name})`, async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: "バックエンド readiness" })).toHaveCount(0);
...
test("OCI 認証プロファイルは未入力でも保存できる", async ({ page }) => {
  let savedPayload: unknown;
  await mockApi(page, {
    onOciSettingsUpdate: (body) => {
      savedPayload = body;
...
test("OCI 認証プロファイルは runtime 由来値を初期表示する", async ({ page }) => {
  await page.addInitScript(() => {
    window.localStorage.setItem(
      "production-ready-rag.oci-settings.v1",
      JSON.stringify({
...
test("OCI config の path と profile から認証プロファイル項目へ反映できる", async ({ page }) => {
  ...
  await expect(page.getByLabel("OCI プロファイル")).toHaveValue("DEFAULT");
  await expect(page.getByLabel("OCI プロファイル")).not.toBeEditable();
