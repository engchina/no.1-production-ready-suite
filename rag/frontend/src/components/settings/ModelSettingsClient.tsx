...
import {
  useModelSettings,
  useTestModelSettings,
  useUpdateModelSettings,
} from "@/lib/queries";
import { cn } from "@/lib/utils";

type CheckKey = keyof NonNullable<ModelSettingsData["checks"]>;
...
const CHECK_MESSAGE_KEYS: Record<
  CheckKey,
  Record<ModelSettingsCheckStatus, I18nKey>
> = {
  enterprise_ai: {
  ...
  const isDirty = useMemo(
    () =>
      Boolean(
        draft &&
        baseline &&
        serializeSettings(draft) !== serializeSettings(baseline),
      ),
    [baseline, draft],
  );
    ...
    try {
      ...
      const message =
        error instanceof ApiError
          ? error.message
          : t("settings.model.test.apiFailed");
      setTestResults((current) => ({
        ...current,
          ...
          >
            <div className="lg:col-span-3">
              <div className="flex flex-wrap items-end justify-between gap-3">
                <div>
                  <h2
                    id="model-status-title"
                    className="text-base font-semibold text-fg"
                  >
                    {t("settings.model.status.title")}
                  </h2>
                  <p className="mt-1 text-sm text-fg-muted">
                    {t("settings.model.status.subtitle")}
                  </p>
                </div>
                <div className="flex flex-wrap items-center gap-2">
                  {isDirty ? (
                    <span className="rounded-full bg-warning-subtle px-3 py-1 text-xs font-medium text-warning-fg">
                      {t("settings.model.unsaved")}
                    </span>
                  ) : null}
                  <span className="rounded-full border border-border bg-surface px-3 py-1 text-xs text-fg-muted">
                    {t("settings.model.source")}:{" "}
                    {t("settings.model.source.runtime")}
                  </span>
                </div>
              </div>
            </div>
              ...
              <Card>
                ...
                <CardContent className="grid gap-5 md:grid-cols-2">
                  <TextField
                    ...
                    onValueChange={(value) =>
                      updateEnterprise("endpoint", value)
                    }
                  ...
                  <TextField
                    ...
                    onValueChange={(value) =>
                      updateEnterprise("api_path", value)
                    }
                    className="md:col-span-2"
                  />
                  <SelectField
                    ...
                    onValueChange={(value) =>
                      updateEnterprise("vlm_input_mode", value)
                    }
                    className="md:col-span-2"
                  />
                  <NumberField
                    ...
                    onChange={(value) =>
                      updateEnterprise("timeout_seconds", value)
                    }
                  />
                  <NumberField
                ...
                <CardContent className="space-y-5">
                  <div className="grid gap-5 md:grid-cols-2">
                    <TestableTextField
                      ...
                      placeholder={t(
                        "settings.model.placeholder.embeddingModel",
                      )}
                      onChange={(value) =>
                    ...
                    <NumberField
                      ...
                      onChange={(value) =>
                        updateGenerative("embedding_dim", value)
                      }
                    />
                    <TestableTextField
                      ...
                      onChange={(value) =>
                        updateGenerative("rerank_model", value)
                      }
                      className="md:col-span-2"
...
function CheckCard({
  ...
  checkKey: CheckKey;
  status: ModelSettingsCheckStatus;
}) {
      ...
      <CardContent className="flex h-full flex-col gap-3 pt-5">
        <div className="flex items-start justify-between gap-3">
          ...
          <Icon
            ...
            className={cn(
              "shrink-0",
              status === "ok" && "text-success-fg",
              status === "missing" && "text-warning-fg",
        ...
        <span
          className={cn(
            ...
            status === "ok" && "bg-success-subtle text-success-fg",
            status === "missing" && "bg-warning-subtle text-warning-fg",
            status === "invalid" && "bg-danger-subtle text-danger-fg",
          )}
        >
          ...
          return (
            ...
            >
              ...
              <Button
                ...
                variant="ghost"
                size="sm"
                className="h-10 w-full px-2 text-danger-fg hover:bg-danger-subtle md:w-10"
    ...
    >
      <div className="flex items-start gap-2.5">
        <Icon
          ...
          className={cn(
            "mt-0.5 shrink-0",
            isSuccess ? "text-success-fg" : "text-danger-fg",
          )}
          aria-hidden
        />
        <div className="min-w-0 flex-1 space-y-2">
          <div>
            <p className="font-medium text-fg">{result.message}</p>
          ...
          {!isSuccess ? (
            <details className="text-xs text-fg">
    ...
    <div className={cn("space-y-1.5", className)}>
      ...
      <div className="relative">
        <input
          ...
          onChange={(event) => onChange(event.target.value)}
          className="h-10 w-full rounded-md border border-border-control bg-surface px-3 pr-12 text-sm text-fg outline-none transition-colors placeholder:text-fg-muted disabled:cursor-not-allowed disabled:opacity-50 focus-visible:border-focus-ring"
        />
        <button
          type="button"
          ...
          className="absolute right-0 top-0 flex h-10 w-10 cursor-pointer items-center justify-center rounded-r-md text-fg-muted transition-colors hover:bg-surface-hover hover:text-fg"
        >
          {visible ? (
...
function FieldLabel({
  ...
  return (
    <div className="flex min-h-5 items-center gap-2">
      ...
      {badge ? (
        <span className="rounded-full bg-info-subtle px-2 py-0.5 text-xs font-medium text-info-fg">
          {badge}
        </span>
      ) : null}
...
function validateDraft(draft: ModelSettingsPayload): string[] {
  ...
  if (
    draft.enterprise_ai.endpoint &&
    !isHttpUrl(draft.enterprise_ai.endpoint)
  ) {
  ...
  const modelIds = draft.enterprise_ai.models.map((model) =>
    model.model_id.trim(),
  );
  const presentModelIds = modelIds.filter(Boolean);
  if (
    draft.enterprise_ai.models.length === 0 ||
    presentModelIds.length !== modelIds.length
  ) {
  ...
  if (
    !draft.enterprise_ai.models.some(
      (model) => model.model_id.trim() && model.vision_enabled,
    )
  ) {
