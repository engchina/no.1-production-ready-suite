...
from docrag.parsing.categories import normalize_category
from docrag.parsing.coordinates import clamp_bbox, pdf_bottom_left_to_image_top_left
...
from docrag.parsing.table_structure import misplaced_cell_repairs, unassigned_table_lines
from docrag.models.layout import LayoutRecord
from docrag.config import Settings
from docrag.parsing.progress import ACTIVE_PAGE_PROGRESS, PageProgress

from docrag.adapters.parsers.base import (
    AdapterAvailability,
    AnalysisContext,
...
def _prepare_docling_env(settings: Settings) -> None:
    """互換起動だけキャッシュ環境を設定する。SDK はホストのプロセス環境を変更しない。"""
...
def _build_docling_converter(settings: Settings):
    ...
    from docrag.adapters.parsers.docling_progress import ProgressPdfPipeline
