...
from docrag.resources.jsonl import record_to_legacy_jsonl
from docrag.parsing.picture_descriptions import (
...
class PictureDescriptionTests(unittest.TestCase):
    ...
    def test_persists_normalized_visual_kind_and_diagram_structure(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("docrag.parsing.picture_descriptions.describe_picture", return_value=description):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("docrag.parsing.picture_descriptions.describe_picture", side_effect=describe):
                stats = describe_docling_pictures(
                    ...
                    settings=get_settings(environ={"OCI_ENTERPRISE_AI_VLM_MODEL": "vendor.vision-1"}, dotenv_path=None),
                )
            ...
            self.assertEqual(record_to_legacy_jsonl(earlier)["sentence"], earlier.text)
    ...
    def test_empty_bbox_is_recorded_as_failure(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("docrag.parsing.picture_descriptions.describe_picture") as describe:
                stats = describe_docling_pictures(
                    [record], [page], run_dir=run_dir, pdf_name="manual.pdf", settings=get_settings()
                )
