...
# - parser は 0.0.0.0:8000 で公開。registry が localhost の vLLM へ OCR を委譲する。
...
# shellcheck disable=SC2086  # DOTS_OCR_VLLM_EXTRA_ARGS は意図的に分割展開する
vllm serve "$DOTS_OCR_MODEL" \
    --served-model-name "$DOTS_OCR_SERVED_MODEL_NAME" \
    --trust-remote-code \
    --chat-template-content-format string \
    --host 127.0.0.1 \
    --port 8080 \
    --tensor-parallel-size "$DOTS_OCR_TENSOR_PARALLEL_SIZE" \
    --gpu-memory-utilization "$DOTS_OCR_GPU_MEMORY_UTILIZATION" \
    $DOTS_OCR_VLLM_EXTRA_ARGS &
