# RAG の Compute 1 台。Docker Compose で backend / ingestion-worker / 前処理 / CPU parser を動かし、
# host の Nginx が frontend の静的 build を配信して /api/ を backend（127.0.0.1:8000）へ proxy する。
# GPU の parser 用 Compute は含めない（将来、GPU shape の oci_core_instance を任意で追加する）。
